...
def compute_volume(span, resolution, lattice_param, f):
    x, y, z = span * numpy.mgrid[-1:1:resolution, -1:1:resolution, -1:1:resolution] * lattice_param
    volume = f(x, y, z)
    vertices, faces, normals, vals = measure.marching_cubes(
        volume, 0, spacing=(0.1, 0.1, 0.1), allow_degenerate=True
    )
...
def make_plot(vertices, faces):
    ...
    ax.plot_trisurf(vertices[:, 0], vertices[:, 1], faces, vertices[:, 2], cmap='ocean', lw=1)
    return pyplot
